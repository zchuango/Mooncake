...
class UbContext {
   ...
   protected:
    static int joinNonblockingPollList(int& event_fd, int data_fd) {
        ...
        if (event_fd < 0) {
            MC_PLOG_ERROR << "Failed to create epoll";
            return ERR_CONTEXT;
        }
        epoll_event event{};
        ...
        if (flags == -1) {
            MC_PLOG_ERROR << "Failed to get file descriptor flags";
            return ERR_CONTEXT;
        }
        if (fcntl(data_fd, F_SETFL, flags | O_NONBLOCK) == -1) {
            MC_PLOG_ERROR << "Failed to set file descriptor nonblocking";
            return ERR_CONTEXT;
        }
        ...
        if (epoll_ctl(event_fd, EPOLL_CTL_ADD, event.data.fd, &event)) {
            MC_PLOG_ERROR << "Failed to register file descriptor to epoll";
            return ERR_CONTEXT;
        }
        return 0;
    }
