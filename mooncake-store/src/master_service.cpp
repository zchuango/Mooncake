...
tl::expected<std::string, ErrorCode> GetGroupIdForKey(
    ...
    if (config.group_ids->size() != key_count || key_index >= key_count) {
        MC_LOG_ERROR << "group_ids.size()=" << config.group_ids->size()
                   << ", key_count=" << key_count
...
MasterService::MasterService(const MasterServiceConfig& config)
    ...
    if (enable_snapshot_ || enable_snapshot_restore_) {
        try {
            ...
            MC_LOG_ERROR << "Failed to create snapshot stores: " << e.what();
    ...
    if (enable_snapshot_ && snapshot_retention_count_ == 0) {
        MC_LOG_ERROR << "snapshot_retention_count must be greater than 0";
        throw std::invalid_argument("snapshot_retention_count must be > 0");
    }
    if (eviction_ratio_ < 0.0 || eviction_ratio_ > 1.0) {
        MC_LOG_ERROR << "Eviction ratio must be between 0.0 and 1.0, "
    ...
    if (put_start_release_timeout_sec_ <= put_start_discard_timeout_sec_) {
        MC_LOG_ERROR << "put_start_release_timeout="
    ...
    if (nof_heartbeat_interval_sec_.count() <= 0) {
        MC_LOG_ERROR << "nof_heartbeat_interval_sec must be positive, current "
    ...
    if (nof_heartbeat_probe_timeout_ms_.count() <= 0) {
        MC_LOG_ERROR << "nof_heartbeat_probe_timeout_ms must be positive, "
    ...
    if (nof_heartbeat_failures_threshold_ == 0) {
        MC_LOG_ERROR << "nof_heartbeat_failures_threshold must be positive";
    ...
    if (config.promotion_on_hit && !enable_offload_) {
        MC_LOG_WARNING << "promotion_on_hit=true was requested but "
    ...
    if (promotion_on_hit_) {
        ...
        MC_LOG_INFO << "Promotion-on-hit mode enabled: LOCAL_DISK-only Gets "
    ...
    MC_LOG_INFO << "action=start_eviction_thread";
    ...
    if (enable_cxl_) {
        ...
        MC_LOG_INFO << "action=start_cxl_global_allocator";
    }
}
...
auto MasterService::MountSegment(const Segment& segment, const UUID& client_id)
    ...
    // otherwise this client cannot be monitored and expired.
    {
        ...
        if (!client_ping_queue_.push(pod_client_id)) {
            MC_LOG_ERROR << "segment_name=" << segment.name
                       << ", error=client_ping_queue_full";
            return tl::make_unexpected(ErrorCode::INTERNAL_ERROR);
        }
    }

    MC_LOG_INFO << "client_id=" << client_id
...
auto MasterService::MountNoFSegment(const NoFSegment& segment,
    ...
    -> tl::expected<void, ErrorCode> {
#ifndef USE_NOF
    MC_LOG_ERROR << "client_id=" << client_id << ", segment_name=" << segment.name
    ...
    LOG_INFO << "NoF segment mount: " << "client_id=" << client_id
              << ", action=mount_segment, segment_name=" << segment.name;
...
auto MasterService::ReMountSegment(const std::vector<Segment>& segments,
    ...
    if (ok_client_.contains(client_id)) {
        MC_LOG_WARNING << "client_id=" << client_id
...
void MasterService::TaskCleanupThreadFunc() {
    ...
    LOG_INFO << "Task cleanup thread stopped";
}
...
auto MasterService::UnmountNoFSegment(const UUID& segment_id,
    ...
    -> tl::expected<void, ErrorCode> {
#ifndef USE_NOF
    MC_LOG_ERROR << "client_id=" << client_id << ", segment_id=" << segment_id
               << ", error=nof_pool_disabled";
    return tl::make_unexpected(ErrorCode::UNAVAILABLE_IN_CURRENT_MODE);
#else
    size_t metrics_dec_capacity = 0;  // to update the metrics
...
auto MasterService::ExistKey(const std::string& key)
    ...
    if (!accessor.Exists()) {
        MC_LOG_INFO << "key=" << key << ", info=object_not_found";
        return false;
    }
...
auto MasterService::QueryIp(const UUID& client_id)
    ...
    if (err != ErrorCode::OK) {
        if (err == ErrorCode::SEGMENT_NOT_FOUND) {
            MC_LOG_INFO << "QueryIp: client_id=" << client_id
                    << " not found or has no segments";
            return tl::make_unexpected(ErrorCode::CLIENT_NOT_FOUND);
        }

        MC_LOG_ERROR << "QueryIp: failed to get segments for client_id="
    ...
    if (unique_ips.empty()) {
        MC_LOG_WARNING << "QueryIp: client_id=" << client_id
...
auto MasterService::BatchReplicaClear(
    ...
    for (const auto& key : object_keys) {
        if (key.empty()) {
            MC_LOG_WARNING << "BatchReplicaClear: empty key, skipping";
            continue;
        }
        MetadataAccessorRW accessor(this, key);
        if (!accessor.Exists()) {
            MC_LOG_WARNING << "BatchReplicaClear: key=" << key
                         << " not found, skipping";
            continue;
        }
        ...
        if (clear_all_segments) {
            ...
            MC_LOG_INFO << "BatchReplicaClear: successfully cleared all replicas "
                       "for key="
            ...
            cleared_keys.emplace_back(key);
            MC_LOG_INFO << "BatchReplicaClear: successfully cleared replicas on "
                       "segment_name="
...
auto MasterService::GetReplicaListByRegex(const std::string& regex_pattern)
    ...
    try {
        ...
        MC_LOG_ERROR << "Invalid regex pattern: " << regex_pattern
                   << ", error: " << e.what();
        return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
    }
...
auto MasterService::GetReplicaList(const std::string& key)
    ...
    bool promotion_eligible = false;
    {
        ...
        if (replica_list.empty()) {
            MC_LOG_WARNING << "key=" << key << ", error=replica_not_ready";
...
auto MasterService::AllocateAndInsertMetadata(
    ...
    if (config.replica_num > 0) {
        ...
        if (!allocation_result.has_value()) {
            MC_LOG_INFO << "Failed to allocate replicas for key=" << key
    ...
    if (config.nof_replica_num > 0 &&
        ...
        if (!allocation_result.has_value()) {
            MC_LOG_INFO << "Failed to allocate nof replicas for key=" << key
                    << ", error: " << allocation_result.error();
            if (allocation_result.error() == ErrorCode::INVALID_PARAMS) {
                return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
            }
    ...
    if (!HasExpectedReplicaAllocation(config, allocated_memory_replicas,
        ...
        MC_LOG_INFO << "Failed to satisfy replica allocation requirement for key="
    ...
    int i = 0;
    MC_LOG_INFO << "PutStart, create replicas: client_id=" << client_id
    ...
    for (const auto& replica : replicas) {
        ...
        if (replica.is_memory_replica()) {
            ...
            MC_LOG_INFO << "Replica #" << ++i << ": buffer_address="
...
auto MasterService::PutStart(const UUID& client_id, const std::string& key,
    ...
    if ((config.replica_num == 0 && config.nof_replica_num == 0) ||
        ...
        MC_LOG_ERROR << "key=" << key << ", replica_num=" << config.replica_num
    ...
    if ((memory_allocator_type_ == BufferAllocatorType::CACHELIB) &&
        ...
        MC_LOG_ERROR << "key=" << key << ", slice_length=" << slice_length
                   << ", max_size=" << kMaxSliceSize
                   << ", error=invalid_slice_size";
        return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
    }

    MC_LOG_INFO << "key=" << key << ", value_length=" << slice_length
    ...
    auto prepare_existing =
        ...
        MC_LOG_INFO << "key=" << key << ", info=object_already_exists";
...
auto MasterService::PutEnd(const UUID& client_id, const std::string& key,
    ...
    MetadataAccessorRW accessor(this, key);
    if (!accessor.Exists()) {
        MC_LOG_ERROR << "key=" << key << ", error=object_not_found";
        return tl::make_unexpected(ErrorCode::OBJECT_NOT_FOUND);
    }

    auto& metadata = accessor.Get();
    if (client_id != metadata.client_id) {
        MC_LOG_ERROR << "Illegal client " << client_id << " to PutEnd key " << key
...
auto MasterService::AddReplica(const UUID& client_id, const std::string& key,
    ...
    if (replica.type() != ReplicaType::LOCAL_DISK) {
        MC_LOG_ERROR << "Invalid replica type: " << replica.type()
...
auto MasterService::PutRevoke(const UUID& client_id, const std::string& key,
    ...
    auto& metadata = accessor.Get();
    if (client_id != metadata.client_id) {
        MC_LOG_ERROR << "Illegal client " << client_id << " to PutRevoke key "
    ...
    if (processing_rep != nullptr) {
        MC_LOG_ERROR << "key=" << key << ", status=" << processing_rep->status()
...
auto MasterService::UpsertStart(const UUID& client_id, const std::string& key,
    ...
    while (true) {
        ...
        if (it == shard->metadata.end()) {
            MC_LOG_INFO << "key=" << key << ", action=upsert_start_case_a";
            const std::string group_id =
        ...
        if (metadata.HasReplica(&Replica::fn_is_busy)) {
            MC_LOG_INFO << "key=" << key << ", error=object_replica_busy";
        ...
        if (metadata.size == slice_length) {
            ...
            MC_LOG_INFO << "key=" << key << ", action=upsert_start_case_b_inplace";
            return replica_list;
        }
        ...
        MC_LOG_INFO << "key=" << key << ", action=upsert_start_case_c_reallocate";
...
MasterService::BatchUpsertStart(const UUID& client_id,
    ...
    if (keys.size() != slice_lengths.size()) {
        MC_LOG_ERROR << "BatchUpsertStart: keys.size()=" << keys.size()
    ...
    if (config.group_ids.has_value() &&
        ...
        MC_LOG_ERROR << "BatchUpsertStart: group_ids.size()="
...
auto MasterService::EvictDiskReplica(const UUID& client_id,
    ...
    MetadataAccessorRW accessor(this, key);
    if (!accessor.Exists()) {
        MC_LOG_INFO << "key=" << key << ", info=object_not_found_for_eviction";
        return tl::make_unexpected(ErrorCode::OBJECT_NOT_FOUND);
    }
...
tl::expected<CopyStartResponse, ErrorCode> MasterService::CopyStart(
    ...
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    {
        ...
        for (const auto& tgt_segment : tgt_segments) {
            if (!segment_access.ExistsSegmentName(tgt_segment)) {
                MC_LOG_ERROR << "key=" << key << ", tgt_segment=" << tgt_segment
    ...
    MetadataAccessorRW accessor(this, key);
    if (!accessor.Exists()) {
        MC_LOG_ERROR << "key=" << key << ", object not found";
        return tl::make_unexpected(ErrorCode::OBJECT_NOT_FOUND);
    }
    ...
    if (source == nullptr || !source->is_completed() ||
        ...
        MC_LOG_ERROR << "key=" << key << ", src_segment=" << src_segment
...
tl::expected<void, ErrorCode> MasterService::CopyEnd(const UUID& client_id,
    ...
    if (task.client_id != client_id) {
        MC_LOG_ERROR << "Illegal client " << client_id << " to CopyEnd key "
                   << key << ", was CopyStart-ed by " << task.client_id;
        return tl::make_unexpected(ErrorCode::ILLEGAL_CLIENT);
    }

    if (task.type != ReplicationTask::Type::COPY) {
        MC_LOG_ERROR << "Ongoing replication task type is MOVE instead of COPY";
        return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
    }
    ...
    if (source == nullptr || !source->is_completed() ||
        source->has_invalid_mem_handle()) {
        MC_LOG_ERROR << "key=" << key << ", source_id=" << source_id
...
tl::expected<void, ErrorCode> MasterService::CopyRevoke(
    ...
    auto& task = accessor.GetReplicationTask();
    if (task.client_id != client_id) {
        MC_LOG_ERROR << "Illegal client " << client_id << " to CopyRevoke key "
                   << key << ", was CopyStart-ed by " << task.client_id;
        return tl::make_unexpected(ErrorCode::ILLEGAL_CLIENT);
    }
    ...
    auto source = metadata.GetReplicaByID(source_id);
    if (source == nullptr) {
        MC_LOG_WARNING << "key=" << key << ", source_id=" << source_id
...
tl::expected<MoveStartResponse, ErrorCode> MasterService::MoveStart(
    ...
    if (src_segment == tgt_segment) {
        MC_LOG_ERROR << "key=" << key << ", move_tgt=" << tgt_segment
...
tl::expected<void, ErrorCode> MasterService::MoveEnd(const UUID& client_id,
    ...
    auto& task = accessor.GetReplicationTask();
    if (task.client_id != client_id) {
        MC_LOG_ERROR << "Illegal client " << client_id << " to MoveEnd key "
                   << key << ", was MoveStart-ed by " << task.client_id;
        return tl::make_unexpected(ErrorCode::ILLEGAL_CLIENT);
    }

    if (task.type != ReplicationTask::Type::MOVE) {
        MC_LOG_ERROR << "Ongoing replication task type is COPY instead of MOVE";
        return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
    }
...
tl::expected<void, ErrorCode> MasterService::MoveRevoke(
    ...
    auto& task = accessor.GetReplicationTask();
    if (task.client_id != client_id) {
        MC_LOG_ERROR << "Illegal client " << client_id << " to MoveRevoke key "
                   << key << ", was MoveStart-ed by " << task.client_id;
        return tl::make_unexpected(ErrorCode::ILLEGAL_CLIENT);
    }
...
auto MasterService::Remove(const std::string& key, bool force)
    ...
    MetadataAccessorRW accessor(this, key);
    if (!accessor.Exists()) {
        MC_LOG_INFO << "key=" << key << ", error=object_not_found";
        return tl::make_unexpected(ErrorCode::OBJECT_NOT_FOUND);
    }
    ...
    if (!force && !metadata.IsLeaseExpired()) {
        MC_LOG_INFO << "key=" << key << ", error=object_has_lease";
     ...
     */
    if (!metadata.AllReplicas(&Replica::fn_is_completed)) {
        MC_LOG_ERROR << "key=" << key << ", error=replica_not_ready";
        return tl::make_unexpected(ErrorCode::REPLICA_IS_NOT_READY);
    }

    if (accessor.HasReplicationTask()) {
        MC_LOG_ERROR << "key=" << key << ", error=object_has_replication_task";
        return tl::make_unexpected(ErrorCode::OBJECT_HAS_REPLICATION_TASK);
    }
...
auto MasterService::RemoveByRegex(const std::string& regex_pattern, bool force)
    ...
    MC_LOG_INFO << "action=remove_by_regex, pattern=" << regex_pattern
...
long MasterService::RemoveAll(bool force) {
    ...
    MC_LOG_INFO << "action=remove_all_objects"
            << ", removed_count=" << removed_count
...
auto MasterService::MountLocalDiskSegment(const UUID& client_id,
    ...
    -> tl::expected<void, ErrorCode> {
    if (!enable_offload_) {
        MC_LOG_ERROR << "	The offload functionality is not enabled";
...
auto MasterService::OffloadObjectHeartbeat(const UUID& client_id,
    ...
    if (local_disk_segment_it == client_local_disk_segment.end()) {
        MC_LOG_ERROR << "Local disk segment not found with client id = "
                   << client_id;
        return tl::make_unexpected(ErrorCode::SEGMENT_NOT_FOUND);
    }
...
auto MasterService::NotifyOffloadSuccess(
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        if (!res && res.error() != ErrorCode::OBJECT_NOT_FOUND) {
            MC_LOG_ERROR << "Failed to add replica: error=" << res.error()
...
tl::expected<void, ErrorCode> MasterService::PushOffloadingQueue(
    ...
    for (const auto& segment_name_it : segment_names) {
        ...
        if (client_id_it == client_by_name.end()) {
            MC_LOG_ERROR << "Segment " << segment_name_it.value() << " not found";
            return tl::make_unexpected(ErrorCode::SEGMENT_NOT_FOUND);
        }
        auto& client_local_disk_segment =
            local_disk_segment_access.getClientLocalDiskSegment();
...
void MasterService::TryPushPromotionQueue(const std::string& key) {
    ...
    if (!push_result) {
        source->dec_refcnt();
        MC_LOG_INFO << "promotion_push_failed key=" << key
    ...
    MC_LOG_INFO << "promotion_queued key=" << key << " size=" << object_size;
}
...
void MasterService::EvictionThreadFunc() {
    MC_LOG_INFO << "action=eviction_thread_started";
    ...
    while (eviction_running_) {
        ...
        if (used_ratio > eviction_high_watermark_ratio_ ||
            ...
            MC_LOG_INFO << "[EVICT-TRIGGER] memory_ratio=" << used_ratio
            ...
            MC_LOG_INFO << "[EVICT-DONE] BatchEvict execution completed.";
    ...
    MC_LOG_INFO << "action=eviction_thread_stopped";
}
...
void MasterService::DiscardExpiredProcessingReplicas(
    ...
    for (auto task_it = shard->replication_tasks.begin();
        ...
        if (metadata_it == shard->metadata.end()) {
            // The key has been removed from metadata. This should be
            // impossible.
            MC_LOG_ERROR << "Key " << task_it->first
    ...
    for (auto task_it = shard->offloading_tasks.begin();
        ...
        MC_LOG_WARNING << "Offloading task expired for key: " << task_it->first;
    ...
    for (auto task_it = shard->promotion_tasks.begin();
        ...
        MC_LOG_WARNING << "Promotion task expired for key: " << task_it->first;
...
void MasterService::SnapshotThreadFunc() {
    LOG_INFO << "[Snapshot] snapshot_thread started";
    while (snapshot_running_) {
        ...
        MC_LOG_INFO << "[Snapshot] Preparing to fork child process, snapshot_id="
                  << snapshot_id;

        // Create pipe for child process logging
        int log_pipe[2];
        if (pipe(log_pipe) == -1) {
            MC_LOG_ERROR << "[Snapshot] Failed to create log pipe: "
        ...
        if (!descriptor) {
            MC_LOG_ERROR << "[Snapshot] Failed to build descriptor before fork, "
                          "snapshot_id="
                       << snapshot_id
        ...
        pid_t pid;
        {
            ...
            pt_snap_lock.Start();
            std::unique_lock<std::shared_mutex> lock(snapshot_mutex_);
            MC_LOG_INFO << "[Snapshot] Locking snapshot mutex, snapshot_id="
                      << snapshot_id;
            pid = fork();
        ...
        if (pid == -1) {
            // Fork failed
            MC_LOG_ERROR << "[Snapshot] Failed to fork child process for state "
                          "persistence: "
                       << strerror(errno) << ", snapshot_id=" << snapshot_id;
            close(log_pipe[0]);
            close(log_pipe[1]);
    ...
    LOG_INFO << "[Snapshot] snapshot_thread stopped";
}
...
void MasterService::WaitForSnapshotChild(pid_t pid,
    ...
    if (flags == -1 || fcntl(log_pipe_fd, F_SETFL, flags | O_NONBLOCK) == -1) {
        MC_LOG_WARNING << "[Snapshot] Failed to set pipe non-blocking: "
                     << strerror(errno);
    }
        ...
        while (true) {
            ...
            if (n > 0) {
                ...
                while ((pos = log_buffer.find('\n')) != std::string::npos) {
                    ...
                    if (!line.empty()) {
                        MC_LOG_INFO << "[Snapshot:Child] " << line;
                    }
                }
            } else {
                break;
    ...
    while (true) {
        ...
        if (result == -1) {
            MC_LOG_ERROR << "[Snapshot] Failed to wait for child process: "
                       << strerror(errno) << ", snapshot_id=" << snapshot_id
                       << ", child_pid=" << pid;
            ...
            if (elapsed >= timeout_seconds) {
                ...
                flush_child_logs();
                if (!log_buffer.empty()) {
                    MC_LOG_INFO << "[Snapshot:Child] " << log_buffer;
                }
...
void MasterService::HandleChildTimeout(pid_t pid,
                                       const std::string& snapshot_id) {
    MC_LOG_WARNING << "[Snapshot] Child process timeout, snapshot_id="
                 << snapshot_id << ", child_pid=" << pid
    ...
    if (kill(pid, SIGTERM) == 0) {
        ...
        if (waitpid(pid, &status, WNOHANG) == 0) {
            ...
            MC_LOG_WARNING << "[Snapshot] Child process still running, force "
                            "killing, snapshot_id="
                         << snapshot_id << ", child_pid=" << pid;
            kill(pid, SIGKILL);
            ...
            MC_LOG_INFO << "[Snapshot] Child process terminated gracefully after "
                         "SIGTERM, snapshot_id="
                      << snapshot_id << ", child_pid=" << pid;
        }
    } else {
        MC_LOG_ERROR << "[Snapshot] Failed to send SIGTERM to child process, "
                      "snapshot_id="
                   << snapshot_id << ", child_pid=" << pid
...
void MasterService::HandleChildExit(pid_t pid, int status,
    ...
    if (WIFEXITED(status)) {
        ...
        if (exit_code != 0) {
            MC_LOG_ERROR << "[Snapshot] Child process exited with error code: "
                       << exit_code << ", snapshot_id=" << snapshot_id
                       << ", child_pid=" << pid;
            MasterMetricManager::instance().inc_snapshot_fail();
        } else {
            MC_LOG_INFO << "[Snapshot] Child process successfully persisted "
                         "state, snapshot_id="
                      << snapshot_id << ", child_pid=" << pid;
        ...
        MC_LOG_ERROR << "[Snapshot] Child process terminated by signal: "
...
void MasterService::RestoreState() {
    auto* snapshot_catalog_store = GetSnapshotCatalogStore();
    if (!snapshot_catalog_store) {
        MC_LOG_ERROR << "[Restore] Snapshot catalog store is not initialized, "
                      "starting fresh";
        return;
    }

    MC_LOG_INFO << "[Restore] Backend info: "
              << snapshot_object_store_->GetConnectionInfo();
    ...
    if (!latest_result) {
        MC_LOG_WARNING << "[Restore] Failed to load latest snapshot marker: "
    ...
    if (!snapshots_result) {
        if (restore_candidates.empty()) {
            MC_LOG_ERROR << "[Restore] Failed to list restorable snapshots: "
                       << toString(snapshots_result.error())
                       << ", starting fresh";
            return;
        }
        MC_LOG_WARNING << "[Restore] Failed to list fallback snapshots: "
                     << toString(snapshots_result.error())
    ...
    if (restore_candidates.empty()) {
        MC_LOG_ERROR << "[Restore] No previous snapshot found, starting fresh";
        return;
    }
    ...
    ResetStateAfterFailedRestoreAttempt();
    MC_LOG_ERROR << "[Restore] Failed to restore from all candidate snapshots "
...
bool MasterService::TryRestoreStateFromSnapshot(
    ...
    auto fail_restore = [&](const std::string& message) {
        MC_LOG_WARNING << "[Restore] Snapshot candidate " << state_id
                     << " is unusable: " << message;
        ResetStateAfterFailedRestoreAttempt();
        return false;
    };
    ...
    try {
        ...
        if (use_snapshot_backup_dir_) {
            ...
            if (!save_result) {
                MC_LOG_ERROR << "[Restore] Failed to save manifest to file: "
                           << save_result.error();
            }
        }
        ...
        MC_LOG_INFO << "[Restore] Trying snapshot: " << state_id
        ...
        if (use_snapshot_backup_dir_) {
            ...
            if (!save_result) {
                MC_LOG_ERROR << "[Restore] Failed to save metadata to file: "
                           << save_result.error();
            }
        }
        LOG_INFO << "[Restore] Download metadata file success";
        ...
        if (use_snapshot_backup_dir_) {
            ...
            if (!save_result) {
                MC_LOG_ERROR << "[Restore] Failed to save segments to file: "
                           << save_result.error();
            }
        }
        LOG_INFO << "[Restore] Download segments file success";
        ...
        if (use_snapshot_backup_dir_) {
            ...
            if (!save_result) {
                MC_LOG_ERROR << "[Restore] Failed to save task manager to file: "
                           << save_result.error();
            }
        }
        MC_LOG_INFO << "[Restore] Download task manager file success";
        ...
        LOG_INFO << "[Restore] Deserialize segments success";
        ...
        LOG_INFO << "[Restore] Deserialize metadata success";
        ...
        MC_LOG_INFO << "[Restore] Deserialize task manager success";
...

        {
            ...
            if (err == ErrorCode::OK) {
                ...
                MC_LOG_INFO << "[Restore] Total capacity size after restore: "
                          << total_size;
            } else {
                MC_LOG_ERROR << "[Restore] Failed to get all segments, error: "
                           << err;
            }
        }

        MC_LOG_INFO << "[Restore] Successfully restored state from snapshot: "
                  << state_id;
        return true;
    } catch (const std::exception& e) {
...
void MasterService::BatchEvict(double evict_ratio_target,
    ...
    if (evict_ratio_target < evict_ratio_lowerbound) {
        MC_LOG_ERROR << "evict_ratio_target=" << evict_ratio_target
    ...
    if (target_evict_num > 0) {
        ...
        if (target_evict_num <= static_cast<long>(no_pin_objects.size())) {
            ...
            MC_LOG_ERROR << "Error in second pass eviction: target_evict_num="
                       << target_evict_num
    ...
    MC_LOG_INFO << "action=evict_objects" << ", evicted_count=" << evicted_count
...
void MasterService::NoFBatchEvict(double evict_ratio_target,
    ...
    if (evict_ratio_target < evict_ratio_lowerbound) {
        MC_LOG_ERROR << "nof_evict_ratio_target=" << evict_ratio_target
    ...
    MC_LOG_INFO << "action=evict_nof_replicas"
            << ", evicted_count=" << evicted_count
            << ", total_freed_size=" << total_freed_size;
        ...
        if (!expired_clients.empty()) {
            ...
            // segment lock.
            {
                ...
                for (size_t i = 0; i < unmount_segments.size(); i++) {
                    ...
                    MC_LOG_INFO << "client_id=" << client_ids[i]
...
bool MasterService::TryUnmountNoFSegmentByHeartbeat(
    ...
    size_t metrics_dec_capacity = 0;
    {
        ...
        if (err == ErrorCode::SEGMENT_NOT_FOUND ||
            ...
            MC_LOG_INFO << "segment_id=" << snapshot.segment_id
                    << ", action=skip_nof_heartbeat_unmount"
                    << ", reason=" << toString(err);
            return false;
        }
        if (err != ErrorCode::OK) {
            MC_LOG_ERROR << "segment_id=" << snapshot.segment_id
...
void MasterService::NofHeartbeatThreadFunc() {
    ...
    while (nof_heartbeat_running_) {
        ...
        if (probe_success) {
            ...
            MC_LOG_INFO << "segment_id=" << probe_target->segment_id
        ...
        MC_LOG_WARNING << "segment_id=" << probe_target->segment_id
                     << ", segment_name=" << probe_target->segment.name
                     << ", endpoint=" << probe_target->segment.te_endpoint
...
MasterService::MetadataSerializer::Deserialize(
    ...
    MC_LOG_INFO << "Restored Replica::next_id_ to " << next_id;
...
MasterService::MetadataSerializer::DeserializeShard(const msgpack::object& obj,
    ...
    for (uint32_t j = 0; j < metadata_array->via.array.size; ++j) {
        ...
        if (!metadata_result) {
            MC_LOG_ERROR << "Failed to deserialize metadata for key: " << key
                       << ": " << metadata_result.error().message;
            continue;
        }
...
tl::expected<UUID, ErrorCode> MasterService::CreateCopyTask(
    ...
    std::shared_lock<std::shared_mutex> shared_lock(snapshot_mutex_);
    if (targets.empty()) {
        MC_LOG_ERROR << "key=" << key << ", error=empty_targets";
        return tl::make_unexpected(ErrorCode::INVALID_PARAMS);
    }
    MetadataAccessorRO accessor(this, key);
    if (!accessor.Exists()) {
    ...
    for (const auto& target : targets) {
        if (!segment_accessor.ExistsSegmentName(target)) {
            MC_LOG_ERROR << "key=" << key << ", target_segment=" << target
    ...
    if (segment_names.empty()) {
        MC_LOG_ERROR << "key=" << key << ", error=no_valid_source_replicas";
        return tl::make_unexpected(ErrorCode::INTERNAL_ERROR);
    }
...
tl::expected<UUID, ErrorCode> MasterService::CreateMoveTask(
    ...
    if (source == target) {
        MC_LOG_ERROR << "key=" << key << ", source_segment=" << source
    ...
    if (error != ErrorCode::OK) {
        MC_LOG_ERROR << "key=" << key << ", segment_name=" << source
                   << ", error=client_id_not_found";
        return tl::make_unexpected(ErrorCode::INTERNAL_ERROR);
    }
...
tl::expected<QueryTaskResponse, ErrorCode> MasterService::QueryTask(
    ...
    if (!task_option.has_value()) {
        MC_LOG_ERROR << "task_id=" << task_id << ", error=task_not_found";
...
tl::expected<void, ErrorCode> MasterService::MarkTaskToComplete(
    ...
    if (err != ErrorCode::OK) {
        MC_LOG_ERROR << "task_id=" << request.id
