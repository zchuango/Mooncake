...
tl::expected<int64_t, ErrorCode> RealClient::get_into_range_internal(
    ...
    if (!metadata_result) {
        if ((metadata_result.error() == ErrorCode::OBJECT_NOT_FOUND ||
            ...
            src_offset == 0) {
            MC_LOG(INFO) << "Object not found for key: " << key;
        }
        auto now = std::chrono::steady_clock::now();
        auto total_us =
...
void ClientRequester::release_offload_buffer(const std::string &client_addr,
    ...
    if (!result) {
        ...
        MC_LOG(INFO) << "Failed to release_offload_buffer for batch_id=" << batch_id
                << " at " << client_addr
                << " (will be GC'd): " << result.error();
    } else {
        MC_LOG(INFO) << "Successfully released buffer for batch_id=" << batch_id
                << " at " << client_addr;
    }
}
