...
void LogRealClientSingleSlow(const char *op, const std::string &key,
    ...
    LOG(WARNING) << op << "_slow key[" << key << "] size[" << size
...
void LogRealClientBatchSlow(const char *op, size_t num_keys, uint64_t size,
    ...
    if (!IsRealClientSlow(latency_us)) return;
    LOG(WARNING) << op << "_slow num_keys[" << num_keys << "] size["
...
int RealClient::put(const std::string &key, std::span<const char> value,
    ...
    LOG(INFO) << "put_start key[" << key << "] size[" << value.size_bytes()
                 << "]";
...
int RealClient::put_batch(const std::vector<std::string> &keys,
                          ...
                          const ReplicateConfig &config) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "put_batch_start num_keys[" << keys.size()
...
int RealClient::put_parts(const std::string &key,
                          ...
                          const ReplicateConfig &config) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "put_parts_start key[" << key << "] total_size["
...
std::shared_ptr<BufferHandle> RealClient::get_buffer(const std::string &key) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "get_buffer_start key[" << key << "]";
...
std::vector<std::shared_ptr<BufferHandle>> RealClient::batch_get_buffer(
    const std::vector<std::string> &keys) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "batch_get_buffer_start num_keys[" << keys.size() << "]";
...
tl::expected<int64_t, ErrorCode> RealClient::execute_ranged_read(
    ...
    if (src_offset == 0 && size == total_size) {
        ...
        if (replica.is_disk_replica()) {
            ...
            if (auto r = scatter_host_to_maybe_device(
                ...
                !r) {
                LOG(ERROR) << "DISK full read scatter failed for key '" << key
                           << "': " << toString(r.error());
    ...
    auto partial_disk_read =
        ...
        if (auto r = scatter_host_to_maybe_device(
            ...
            !r) {
            LOG(ERROR) << "Ranged disk read scatter failed for key '" << key
                       << "': " << toString(r.error());
            return tl::unexpected(r.error());
        }
    ...
    if (replica.is_local_disk_replica()) {
        ...
        return partial_disk_read(
            [&](void *tmp_buf) -> tl::expected<void, ErrorCode> {
                ...
                pt_ssd.End(result ? 0 : -1);
                if (!result) {
                    LOG(ERROR) << "Ranged SSD read failed for key '" << key
                               << "': " << toString(result.error());
                }
                return result;
            },
    ...
    pt_read.End(get_result ? 0 : -1);
    if (!get_result) {
        LOG(ERROR) << "Ranged Get failed for key: " << key
                   << " with error: " << toString(get_result.error());
        return tl::unexpected(get_result.error());
    }
    return static_cast<int64_t>(size);
...
tl::expected<int64_t, ErrorCode> RealClient::get_into_range_internal(
    ...
    if (!metadata_result) {
        if ((metadata_result.error() == ErrorCode::OBJECT_NOT_FOUND ||
            ...
            src_offset == 0) {
            LOG(INFO) << "Object not found for key: " << key;
        }
        auto now = std::chrono::steady_clock::now();
        auto total_us =
            std::chrono::duration_cast<std::chrono::microseconds>(now - t0)
                .count();
        LOG(INFO) << "get_into_breakdown key[" << key
    ...
    auto total_us = std::chrono::duration_cast<std::chrono::microseconds>(
                        read_end - t0)
                        .count();
    LOG(INFO) << "get_into_breakdown key[" << key << "] query_us["
...
int64_t RealClient::get_into(const std::string &key, void *buffer,
                             size_t size) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "get_into_start key[" << key << "] size[" << size << "]";
...
std::vector<int> RealClient::batch_put_from(
    ...
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "batch_put_from_start num_keys[" << keys.size()
...
int RealClient::put_from(const std::string &key, void *buffer, size_t size,
                         const ReplicateConfig &config) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "put_from_start key[" << key << "] size[" << size << "]";
    auto result = execute_timed_operation<tl::expected<void, ErrorCode>>(
...
std::vector<int64_t> RealClient::batch_get_into(
    ...
    const std::vector<size_t> &sizes) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "batch_get_into_start num_keys[" << keys.size()
                 << "] total_size[" << sum_sizes(sizes) << "]";
    auto internal_results =
...
RealClient::batch_get_into_internal(const std::vector<std::string> &keys,
    ...
    LOG(INFO) << "batch_get_into_query_result num_keys[" << num_keys
              << "] num_found[" << num_found << "]";
        ...
        if (!disk_batch_keys.empty()) {
            ...
            for (size_t di = 0; di < disk_batch_indices.size(); ++di) {
                ...
                if (auto r = scatter_host_to_maybe_device(
                    ...
                    !r) {
                    disk_read_success = false;
                    LOG(ERROR) << "DISK scatter failed for key '" << key
                               << "': " << toString(r.error());
    ...
    for (auto &offload_objects_it : offload_objects) {
        ...
        if (!batch_get_offload_result) {
            LOG(ERROR) << "Batch get store object failed endpoint["
...
std::vector<int> RealClient::batch_put_from_multi_buffers(
    ...
    const ReplicateConfig &config) {
    mooncake::logging::ScopedTraceId trace(mooncake::logging::NewTraceId());
    LOG(INFO) << "batch_put_from_multi_buffers_start num_keys["
...
void ClientRequester::release_offload_buffer(const std::string &client_addr,
    ...
    if (!result) {
        ...
        LOG(INFO) << "Failed to release_offload_buffer for batch_id=" << batch_id
                << " at " << client_addr
                << " (will be GC'd): " << result.error();
    } else {
        LOG(INFO) << "Successfully released buffer for batch_id=" << batch_id
                << " at " << client_addr;
    }
}
