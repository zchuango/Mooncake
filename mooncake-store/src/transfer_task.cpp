...
FilereadWorkerPool::FilereadWorkerPool(std::shared_ptr<StorageBackend>& backend)
    : shutdown_(false) {
    DLOG(DEBUG) << "Creating FilereadWorkerPool with "
...
FilereadWorkerPool::~FilereadWorkerPool() {
    ...
    DLOG(DEBUG) << "FilereadWorkerPool destroyed";
}
...
void FilereadWorkerPool::workerThread() {
    DLOG(TRACE) << "FilereadWorkerPool worker thread started";
    ...
    while (true) {
        ...
        if (task.state) {
            ...
            try {
                ...
                if (load_result) {
                    DLOG(TRACE) << "Fileread task completed successfully with "
                               << task.file_path;
                ...
                LOG_ERROR << "Exception during async fileread: " << e.what();
                task.state->set_completed(ErrorCode::TRANSFER_FAIL);
            }
        }
    }

    DLOG(TRACE) << "FilereadWorkerPool worker thread exiting";
}
...
SpdkNofWorkerPool::SpdkNofWorkerPool(int numa_socket_id)
    : worker_count_(GetSpdkNofWorkerCount()),
      ...
      shutdown_(false) {
    DLOG(DEBUG) << "Creating SpdkNofWorkerPool with " << worker_count_
               << " workers";
...
SpdkNofWorkerPool::~SpdkNofWorkerPool() {
    ...
    DLOG(DEBUG) << "SpdkNofWorkerPool destroyed";
}
...
void SpdkNofWorkerPool::submitTask(SpdkNofTask task) {
    ...
    int worker_idx = -1;
    {
        ...
        if (new_binding && IsSpdkNofDebugEnabled()) {
            DLOG(INFO) << "nof_worker_bind seg_handle=" << seg
...
void SpdkNofWorkerPool::workerThread(int work_idx) {
    ...
    DLOG(TRACE) << "SpdkNofWorkerPool worker thread started";
    ...
    DLOG(TRACE) << "SpdkNofWorkerPool worker thread exiting";
}
#endif
...
MemcpyWorkerPool::MemcpyWorkerPool() : shutdown_(false) {
    DLOG(DEBUG) << "Creating MemcpyWorkerPool with " << kDefaultMemcpyWorkers
               << " workers";
...
MemcpyWorkerPool::~MemcpyWorkerPool() {
    ...
    DLOG(DEBUG) << "MemcpyWorkerPool destroyed";
}
...
void MemcpyWorkerPool::workerThread() {
    DLOG(TRACE) << "MemcpyWorkerPool worker thread started";
    ...
    while (true) {
        ...
        if (task.state) {
            ...
            try {
                ...
                DLOG(TRACE) << "Memcpy task completed with "
                           << task.operations.size() << " operations"
                           << (ok ? "" : " (with GPU copy failure)");
                task.state->set_completed(ok ? ErrorCode::OK
                                             : ErrorCode::TRANSFER_FAIL);
            } catch (const std::exception& e) {
                LOG_ERROR << "Exception during async memcpy: " << e.what();
                task.state->set_completed(ErrorCode::TRANSFER_FAIL);
            }
        }
    }

    DLOG(TRACE) << "MemcpyWorkerPool worker thread exiting";
}
...
void TransferEngineOperationState::check_task_status() {
    ...
    for (size_t i = 0; i < batch_size_; ++i) {
        ...
        if (!s.ok()) {
            LOG_ERROR << "Failed to get transfer status for batch "
        ...
        switch (status.s) {
            ...
            case TransferStatusEnum::INVALID:
                ...
                DLOG(DEBUG) << "Transfer failed for batch " << batch_id_
                           << " task " << i << " with status "
...
void TransferEngineOperationState::set_result_internal(ErrorCode error_code) {
    [[maybe_unused]] auto trace = RestoreTraceIfMissing(trace_id());
    if (result_.has_value()) {
        LOG_ERROR << "Attempting to set result multiple times for batch "
                   << batch_id_
    ...
    DLOG(DEBUG) << "Setting transfer result for batch " << batch_id_ << " to "
...
void TransferEngineOperationState::wait_for_completion() {
    ...
    DLOG(DEBUG) << "Waiting for transfer engine completion for batch "
               << batch_id_;
    ...
    if (completed) {
        DLOG(DEBUG) << "Transfer engine operation completed for batch "
                   << batch_id_
...
#else
    DLOG(DEBUG) << "Starting transfer engine polling for batch " << batch_id_;
    ...
    while (true) {
        ...
        // Continue polling
        DLOG(DEBUG) << "Transfer engine operation still pending for batch "
                   << batch_id_;
    }
#endif
}
...
TransferFuture::TransferFuture(std::shared_ptr<OperationState> state)
    ...
    if (!state_) {
        LOG_ERROR << "TransferFuture requires valid state";
...
ErrorCode TransferFuture::wait() {
    ...
    static std::atomic<bool> first_wait_flag{true};
    const bool first_wait = first_wait_flag.exchange(false);
    ...
    const auto wait_us = std::chrono::duration_cast<std::chrono::microseconds>(
                             ...
                             .count();
    DLOG(INFO) << "transfer_future_wait first_wait[" << (first_wait ? 1 : 0)
...
TransferSubmitter::TransferSubmitter(TransferEngine& engine,
    ...
    if (env_value == nullptr) {
        ...
        if (env_str == "false" || env_str == "0" || env_str == "no" ||
            ...
            LOG_WARNING << "Invalid value for MC_STORE_MEMCPY: " << env_str
                         << ", defaulting to enabled";
            memcpy_enabled_ = true;
        }
    }

    DLOG(DEBUG) << "TransferSubmitter initialized with memcpy_enabled="
               << memcpy_enabled_;
}
        ...
        if (op_code == TransferRequest::READ) {
            ...
            switch (strategy) {
                ...
                case TransferStrategy::TRANSFER_ENGINE:
                    ...
                    break;
                default:
                    LOG_ERROR << "Unknown transfer strategy: " << strategy;
                    return std::nullopt;
            }
        }
    ...
    for (size_t i = 0; i < replicas.size(); ++i) {
        ...
        const auto open_segment_us =
            std::chrono::duration_cast<std::chrono::microseconds>(
                ...
                .count();
        DLOG(INFO) << "open_segment_breakdown endpoint["
...
TransferSubmitter::submit_batch_get_offload_object(
    ...
    const auto open_segment_us =
        std::chrono::duration_cast<std::chrono::microseconds>(
            std::chrono::steady_clock::now() - t_open_start)
            .count();
    DLOG(INFO) << "open_segment_breakdown endpoint[" << transfer_engine_addr
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
        ...
        if (it == batched_slices.end()) {
            LOG_ERROR << "Key not found in batched_slices: " << key;
...
std::optional<TransferFuture> TransferSubmitter::submitMemcpyOperation(
    ...
    DLOG(DEBUG) << "Memcpy transfer submitted to worker pool with "
...
std::optional<TransferFuture> TransferSubmitter::submitTransfer(
    std::vector<TransferRequest>& requests) {
    const auto t0 = std::chrono::steady_clock::now();
    static std::atomic<bool> first_transfer_flag{true};
    const bool first_transfer = first_transfer_flag.exchange(false);
    ...
    DLOG(INFO) << "submit_transfer_breakdown first_transfer["
    ...
    if (batch_id == INVALID_BATCH_ID) {  // INVALID_BATCH_ID
        LOG_ERROR << "Invalid batch ID for transfer engine operation";
        return std::nullopt;
    }
...
std::optional<TransferFuture> TransferSubmitter::submitTransferEngineOperation(
    ...
    if (handle.transport_endpoint_.empty()) {
        LOG_ERROR << "Transport endpoint is empty for handle with address "
...
std::optional<TransferFuture> TransferSubmitter::submitMemoryReadOperation(
    ...
    LOG_ERROR << "Read only supports LOCAL_MEMCPY or TRANSFER_ENGINE, got: "
               << strategy;
    return std::nullopt;
}
...
std::optional<TransferFuture> TransferSubmitter::submitRangeRead(
    ...
    if (replica.is_memory_replica()) {
        ...
        if (src_offset + slices_size > handle.size_) {
            LOG_ERROR << "Range read overflow: src_offset=" << src_offset
...
std::optional<TransferFuture> TransferSubmitter::submitSpdkNofOperation(
    ...
    DLOG(DEBUG) << "SPDK NoF transfer submitted to "
               << handle.transport_endpoint_;
    return TransferFuture(state);
}
#endif
...
std::optional<TransferFuture> TransferSubmitter::submitFileReadOperation(
    ...
    DLOG(DEBUG) << "Fileread transfer submitted to worker pool with "
               << file_path;
...
std::string extractIpAddress(const std::string& endpoint) {
    ...
    if (endpoint[0] == '[') {
        ...
        if (closing_bracket == std::string::npos) {
            LOG_WARNING << "Invalid IPv6 endpoint format: " << endpoint;
            return "";
        }
...
bool TransferSubmitter::isSameProcessEndpoint(
    ...
    if (!handle_ip.empty() && handle_ip == local_ip) {
        DLOG(TRACE) << "Disabling local memcpy for same-host endpoints with "
...
bool TransferSubmitter::validateTransferParams(
    ...
    if (handle.size_ != all_slice_len) {
        LOG_ERROR << "handles len:" << handle.size_
