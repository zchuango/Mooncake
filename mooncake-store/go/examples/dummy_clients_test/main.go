...
func createAndSetupStore(workerID int) *store.Store {
	...
	localhostName := envOrDefault("MC_LOCALHOST", "localhost")
	protocol := envOrDefault("MC_PROTOCOL", "tcp")
	deviceName := envOrDefault("MC_DEVICENAME", "")
	...
	log.Printf("[WORKER-%d]   Metadata Server: %s", workerID, metadataServer)
	log.Printf("[WORKER-%d]   Master Address:   %s", workerID, masterAddr)
	log.Printf("[WORKER-%d]   Protocol:         %s", workerID, protocol)
